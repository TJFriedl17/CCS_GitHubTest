...
void uart_interrupt_init(void){
  ...
  UART1_CTL_R |= 0x301; //001100000001
    ...
    if (UART1_MIS_R & 0b00010000)
    {
        ...
        else
        {
            ...
            else if (byte_received == 'd')
            {
                ...
                command_flag = 3; // Used for right turn
            }
            ...
            else if (byte_received == 'o')
            {
                uart_sendStr("CHOOSING AUTOMATIC MODE");
                uart_sendChar('\n');
                command_flag = 5;
            }
            else if (byte_received == 'p')
            {
                uart_sendStr("CHOOSING MANUAL MODE");
                uart_sendChar('\n');
                command_flag = 6;
            }
            else {
